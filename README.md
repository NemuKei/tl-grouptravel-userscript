...
- 集計サマリーは `対象期間`、`前年同時期(選択条件内)`、`前年同時期(全体基準)` の 3 行で比較できる
- 一覧には実績室数、延人数、Wash率、室料金合計、室単価も表示する
- 円グラフは総合計料金の上位項目を表示し、凡例に当年 / 前年の全体比と売上を表示する
- 円グラフの hover では `販売先`、`当年`、`前年` の 3 段 tooltip を表示し、室数、Wash率、室単価も確認できる
- 一覧の並び順は総合計料金の降順固定
...
- 催行率、Wash率、室単価は分割取得後の合算値から再計算する
...
- 実施済み: 4 種類のクイック選択、前年同時期比較、一覧の売上順、凡例、tooltip、全体サマリーの 3 行レイアウトと配色、全体基準の表示を実サイトで確認

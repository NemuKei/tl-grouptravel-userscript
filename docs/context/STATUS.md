...
- 画面表示だけ全体基準の分母を追加する差分実装は完了し、GUI verify 待ち
...
- 実施済み: 全体サマリーの 2 行レイアウトと前年比較カード配色の実サイト確認
- 未実施: 画面表示だけ全体基準の分母を追加した差分の実サイト GUI 確認
- 確認メモ: CDP で開いている統計データ画面を確認した時点では、ページ上の userscript は旧版のままで、新しい全体基準ラベルは未反映だった
